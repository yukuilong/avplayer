...
EXPORT_API int android_gles_init_video(struct vo_context *ctx, int w, int h, int pix_fmt)
{
	android_gles_render *ogles = NULL;
	ctx->priv = (void*)(ogles = new android_gles_render);
	return ogles->init_render(ctx->user_data, w, h, pix_fmt) ? 0 : -1;
}
EXPORT_API int android_gles_render_one_frame(struct vo_context *ctx, AVFrame* data, int pix_fmt, double pts)
{
	android_gles_render *ogles = (android_gles_render*)ctx->priv;
	return ogles->render_one_frame(data, pix_fmt) ? 0 : -1;
}
EXPORT_API void android_gles_re_size(struct vo_context *ctx, int width, int height)
{
	android_gles_render *ogles = (android_gles_render*)ctx->priv;
	ogles->re_size(width, height);
}
EXPORT_API void android_gles_aspect_ratio(struct vo_context *ctx, int srcw, int srch, int enable_aspect)
{
	android_gles_render *ogles = (android_gles_render*)ctx->priv;
	ogles->aspect_ratio(srcw, srch, (bool)enable_aspect);
}
EXPORT_API int android_gles_use_overlay(struct vo_context *ctx)
{
	android_gles_render *ogles = (android_gles_render*)ctx->priv;
	return ogles->use_overlay() ? 0 : -1;
}
EXPORT_API void android_gles_destory_render(struct vo_context *ctx)
{
	android_gles_render *ogles = (android_gles_render*)ctx->priv;
	if (ogles)
	{
		ogles->destory_render();
		delete ogles;
		ctx->priv = NULL;
	}
}
