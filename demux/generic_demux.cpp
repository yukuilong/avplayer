...
bool generic_demux::open(boost::any ctx)
{
	...
	else
	{
		...
		else if (m_source_ctx->type == MEDIA_TYPE_RTMP)
			strcpy(url, m_source_ctx->info.rtmp.url);
		else
			goto FAILED_FLG;
		...
		if (ret < 0)
		{
			...
			__android_log_print(ANDROID_LOG_INFO,"init","error:%s\n",buf);
#endif
			std::cerr << "av_open_input_stream call failed!\n";
			goto FAILED_FLG;
		}
