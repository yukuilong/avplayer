...
/* MEDIA_TYPE_RTMP文件信息, 暂无实现, 依赖ffmpeg内部实现. */
typedef struct rtmp_source_info
{
	char url[MAX_URI_PATH];
} rtmp_source_info;
...
typedef union source_info
{
	...
	struct rtmp_source_info rtmp;	/* rtmp_source_info类型. */
} source_info;
...
typedef struct source_context
{
	 ...
	 * MEDIA_TYPE_YK
	 * MEDIA_TYPE_RTMP
	 * 说明: 由于作者未实现http和rtsp,rtmp协议而是直接使用了ffmpeg的demux, 所以就无需初始
	 * 化上面的各函数指针.
	 */
	int type;
